...
export default function MediaUploadedItems({
	...
	const handleLocalImageError = (file: File) => {
	...
	const getPreviewUrlSafe = (file: File): string | null => {
	...
	const fileItems = allFiles.map((item, index) => {
		...
		const fileNameWithoutExtension = removeFileExtension(file.name || "unknown-file");
		...
		const previewUrl = getPreviewUrlSafe(file);
		const fileTypeIcon = getFileTypeIcon(file);

		// Get upload status and progress
		const uploadStatus = file.status || "pending";
		...
		return (
			<div
				key={`${file.name || "unknown"}-${index}-${file.size || 0}`}
							...
							{isImage && previewUrl ? (
								<div className="relative h-12 w-12 rounded-md border">
									<Image
										...
										sizes="48px"
										unoptimized={true}
										onError={() => handleLocalImageError(file)}
										onLoad={() => {
									...
									{/* Status overlay */}
									<div
										className={`absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full ${getStatusColor(file)}`}
									>
										{getStatusIcon(file)}
									</div>
								</div>
						...
						<div className="min-w-0 flex-1">
							<div className="mb-1 flex items-center space-x-2">
								<p
									...
									title={file.name || "Unknown file"} // Show full name on hover
								>
									{truncatedFileName}
								</p>
								<Badge
									variant="secondary"
									className={`flex-shrink-0 text-xs ${getStatusBadge(file)}`}
								>
		...
		<div className="mx-auto w-full max-w-4xl space-y-6">
			...
			<div className="flex items-center justify-between">
				<h3 className="text-lg font-semibold">
					Upload Results ({acceptedFiles.length + rejectedFiles.length})
				</h3>
				<Button
					variant="outline"
					size="sm"
			...
			<Card className="pb-5">
				...
				<CardContent>
					<ScrollArea className="md:h-20 lg:h-48">
						<div className="max-w-full space-y-3">{fileItems.filter(item => item !== null)}</div>
